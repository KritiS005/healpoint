...
class AnimationController {
    ...
    private readonly cameraTravelDistance = 4200
    private readonly startDotYOffset = 34
    private readonly viewZoom = 118
    private readonly numberOfStars = 6500
    private readonly trailLength = 110
    ...
    public setPointer(x: number, y: number) {
        this.pointerOffset = new Vector2D(x, y)
    }
    ...
    public spiralPath(p: number): Vector2D {
        ...
        const r = 220 * Math.sqrt(p)
        const pointerX = this.pointerOffset.x * 44
        const pointerY = this.pointerOffset.y * 28

        return new Vector2D(
            r * Math.cos(theta) + pointerX,
            r * Math.sin(theta) + this.startDotYOffset + pointerY
        )
    }
    ...
    public render() {
        ...
        bgGradient.addColorStop(0, 'rgba(34, 211, 238, 0.22)')
        bgGradient.addColorStop(0.3, 'rgba(59, 130, 246, 0.14)')
        bgGradient.addColorStop(0.7, 'rgba(15, 23, 42, 0.92)')
        ...
        ctx.translate(this.size / 2 + this.pointerOffset.x * 34, this.size / 2 + this.pointerOffset.y * 24)
    ...
    private drawTrail(t1: number) {
        for (let i = 0; i < this.trailLength; i++) {
            ...
            const sw = (1.7 * (1 - t1) + 4.2 * Math.sin(Math.PI * t1)) * f

            this.ctx.fillStyle = 'rgba(224, 242, 254, 0.96)'
            this.ctx.lineWidth = sw

            const pathTime = t1 - 0.00012 * i
    ...
    render(p: number, controller: AnimationController) {
        ...
        if (q > 0) {
            ...
            if (displacementProgress < 0.3) {
                ...
                const targetDistance = this.distance * this.expansionRate * 2.2;
            ...
            const dotSize = 14.5 * this.strokeWeightFactor * sizeMultiplier;
...
export function SpiralAnimation() {
    ...
    const updatePointer = (clientX: number, clientY: number, rect: DOMRect) => {
        ...
        animationRef.current?.setPointer(x, y)
    }
    ...
    return (
        <div className="relative h-full w-full overflow-hidden">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,_rgba(34,211,238,0.22),_transparent_34%),radial-gradient(circle_at_bottom_right,_rgba(59,130,246,0.24),_transparent_32%)]" />
            <div className="absolute inset-0 bg-[linear-gradient(110deg,_rgba(255,255,255,0.06)_0%,_transparent_30%,_rgba(255,255,255,0.04)_70%,_transparent_100%)]" />
            <div className="absolute inset-0 bg-[radial-gradient(circle,_rgba(255,255,255,0.12)_0%,_transparent_60%)]" />
            <div
                ...
                onMouseLeave={() => animationRef.current?.setPointer(0, 0)}
            >
                <canvas
                    ref={canvasRef}
